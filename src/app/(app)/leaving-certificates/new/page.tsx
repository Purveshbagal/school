...
import { LeavingCertificateForm } from "./leaving-certificate-form";
import { ArrowLeft, Search } from "lucide-react";

export default async function NewLeavingCertificatePage({
  searchParams,
}: {
  ...
  if (studentId) {
    ...
    return (
      <div className="mx-auto max-w-3xl">
        <PageHeader
          title="Create Leaving Certificate"
          description={`${student.name} · ${student.admissionNo}`}
          actions={
            <Button
              variant="outline"
              size="sm"
              render={<Link href="/leaving-certificates/new">Change Student</Link>}
            >
  ...
  return (
    <div className="mx-auto max-w-lg">
      <PageHeader
        ...
        actions={
          <Button variant="outline" size="sm" render={<Link href="/leaving-certificates">Cancel</Link>} />
        }
      />
          ...
          {students.length === 0 ? (
            ...
            <div className="space-y-2">
              {students.map((s) => (
                <Link
                  key={s.id}
                  href={`/leaving-certificates/new?student=${s.id}`}
